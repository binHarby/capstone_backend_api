#!/usr/bin/env python
from fastapi import FasAPI, Response, status, HTTPException, APIRouter,Depends
...
def get_food_history(user_food_history: schemas.GetFoodReq,get_current_user: int = Depends(oauth.get_current_user)):
    ...
    if user_food_history.frm.date()>=user_food_history.too.date():
        cursor.execute('''SELECT * FROM user_food WHERE id=%s AND created_at>=%s AND created_at<=%s''',(get_current_user.id,user_food_history.frm,
...
def post_food(user_food_history: schemas.PostFoodReq):
